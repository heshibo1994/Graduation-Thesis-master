...
\begin{figure}[H] % use float package if you want it here
  \centering
  \includegraphics[height=9cm]{getVolume_inputCamera_1.png}
  \caption{三维重建输入视频序列图}
...
\subsection{估计场景实际尺度}
\label{sec:5.4.2}
1.	获取绝对尺度:根据图所示,分别可以获得在ID = 43的二维码坐标系下的相机位姿分别为[-0.127325, -0.172455, 0.980864]
和[0.324499, -0.0655995, 0.969803]。
\begin{figure}[H] % use float package if you want it here
  \centering
  ...
  \end{figure}
可以计算出在这两帧之间相机移动的绝对距离为
\begin{equation}Pose_{\_abs}\;=\sqrt{{\lbrack(-0.127325)-0.324499\rbrack}^2+\left[(-0.172455)-(-0.0655995)\right]^2+\left[0.980864-\;0.969803\right]^2}\\
=0.464m\end{equation}
